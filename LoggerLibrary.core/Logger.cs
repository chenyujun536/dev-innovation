    ...
    public static class Logger
    {
        ...
        static Logger()
        {
            ...
            catch (Exception e)
            {
                ...
                logFile= @"C:\\ProgramData\\Schlumberger\\sampletestlog.txt";
            }
        ...
        private static void LogLine(string line, bool debug)
        {
            lock (LockObject)
            {
                if (!debug)
                    Console.WriteLine(
                        $"[{DateTime.Now:HH-mm-ss.fff}] [Thread {Thread.CurrentThread.ManagedThreadId}] [INFO] {line}");

                if (LogSw != null)
                {
                    LogSw?.WriteLine(
                        $"[{DateTime.Now:HH-mm-ss.fff}] [Thread {Thread.CurrentThread.ManagedThreadId}] {(debug ? "[DEBUG]":"[INFO]")} {line}");
                    LogSw?.Flush();
                }
            }
        }
